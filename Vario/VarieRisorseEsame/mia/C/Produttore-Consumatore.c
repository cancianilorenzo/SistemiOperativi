...
int main()
{
    ...
    /*pthread_mutex_init(&m, NULL);      //senza attributi
    pthread_mutex_init(&c_cons, NULL); //senza attributi
    pthread_mutex_init(&c_prod, NULL); //senza attributi*/
...
{
    int val;
    while (1) /*se cambiato con "val != 20" esce quando legge il 20° valore*/
    {
        pthread_mutex_lock(&m);
        if (num < 0)
        {
