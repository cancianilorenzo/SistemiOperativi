...
void main(int argc, char **argv)
{
    ...
    for (int i = 0; i < figli; i++)
    {
        ...
        if (processo == 0)
        {
            ...
            {
                char buffer[1];
                read(fd_master[i][0], &buffer, sizeof(buffer));
                if (buffer[0] == 'i')
                {
                    printf("scrivo pid\n");
                    char pid[sizeof(getpid()) + 1];
                    sprintf(pid, "%d", getpid());
                    if (write(fd_figlio[i][1], pid, strlen(pid)) == -1)
                ...
                {
                    srand(time(
# 54 "CommunicationControl.c" 3 4
                              ((void *)0)
# 54 "CommunicationControl.c"
                                  ));
    ...
    while (1)
    {
        ...
        printf("Comando: ");
        fflush(
# 81 "CommunicationControl.c" 3 4
              stdout
# 81 "CommunicationControl.c"
                    );
        ...
        if (buffer[0] == 'i' || buffer[0] == 'r')
        {
            ...
            if(figlio <= figli){
            write(fd_master[figlio - 1][1], &buffer[0], 1);

            char ingresso[100];
            read(fd_figlio[figlio - 1][0], ingresso, sizeof(ingresso));
        ...
        if (buffer[0] == 'q')
        {
            for (int i = 0; i < figli; i++)
            {
                write(fd_master[i][1], &buffer[0], 1);
            }
            printf("Terminazione di tutti i figli in corso....");
            while (wait(
# 113 "CommunicationControl.c" 3 4
                       ((void *)0)
# 113 "CommunicationControl.c"
                           ) > 0);
            printf("Completeto!\n");
            exit(0);
        }
